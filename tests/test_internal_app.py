...
def test_get_token_invalid_username(client):
    response = client.post('/token', data={'username': 'nao_existe', 'password': 'qualquer_senha'})
    assert response.status_code == status.HTTP_401_UNAUTHORIZED
    assert response.json() == {'detail': 'Incorrect email or password'}


def test_get_token_invalid_password(client, user):
    response = client.post('/token', data={'username': user.username, 'password': 'senha_incorreta'})
    assert response.status_code == status.HTTP_401_UNAUTHORIZED
    assert response.json() == {'detail': 'Incorrect email or password'}
